...
⚠️ **Nota tecnica sulle Recensioni**: la prima volta che qualcuno apre la pagina di un prodotto con recensioni, Firestore potrebbe richiedere una piccola configurazione automatica ("indice composito") — se succede, nella Console del browser (F12) comparirà un errore con un link diretto: clicca quel link, Firebase crea l'indice da solo in circa un minuto, poi tutto funziona normalmente. Capita una sola volta. Lo stesso vale per la pagina Recensioni generale: se vedi "Impossibile caricare le recensioni al momento" senza un link cliccabile in Console, crea l'indice a mano da Firebase Console → Firestore Database → scheda "Indici" → Crea indice → collezione `reviews`, campo `status` Ascendente + campo `createdAt` Discendente, ambito Collection.
- **Banner promozionale scorrevole**: nuova tab "Banner promozionale" nella Dashboard — attivalo, scrivi il testo (es. "Spedizioni gratuite sopra i 150€"), opzionalmente un link. Appare subito in cima a ogni pagina del sito, scorrevole stile "marquee", con una ✕ per chiuderlo (ricompare alla visita successiva). Si disattiva togliendo la spunta "Banner attivo".
- **Dashboard riorganizzata**: le 13 sezioni ora sono in un menu laterale raggruppato per categoria (Catalogo, Contenuti, Community, Impostazioni) invece di una lunga fila di pulsanti — più semplice da navigare, specialmente ora che sono cresciute nel tempo.
- **Forum della community** (`forum.html`, aggiunto al menu): chiunque abbia un account può aprire un nuovo argomento o rispondere a uno esistente, con messaggi in tempo reale e la possibilità di allegare un'immagine (tramite URL, es. da Imgur — non è un vero upload da dispositivo, per restare gratis senza attivare Firebase Storage). Chi non ha fatto login può comunque **leggere** tutto il Forum, ma per scrivere serve un account.
  - **Tag staff**: dalla Dashboard (tab "Forum & Tag Staff") assegni a un'email dello staff un'etichetta e un colore a scelta — compare come badge accanto al nome ogni volta che quella persona scrive nel Forum.
  - **Moderazione**: non c'è approvazione preventiva (bloccherebbe la naturalezza della chat), ma ogni messaggio può essere eliminato dal suo stesso autore o dall'admin, direttamente dalla pagina del Forum (bottone "Elimina" visibile solo a chi ha il permesso). Un intero argomento può essere eliminato dal suo autore o dall'admin.
